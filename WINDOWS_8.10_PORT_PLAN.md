...
|----|-----------|--------|-----------|--------------------|
| 0.1 | M0 | Not started | | `win-8.10` from `8.10.0`; LICENSE intact; `win-5.0` untouched |
...
| 3.3 | `port: parent-only redisFork + do_rdbSave + waitpid table` | `server.c` `redisFork`; `rdb.c` thin `#ifdef`; `Win32_QFork_impl.c`; `Win32_ProcessTable.c`; lift `Win32_CommandLine` pre-parse; `createBoolConfig("persistence-available")` seeded from `g_argMap` | 3.2 | Two-phase persistence-available. `waitpid(server.child_pid)`. `BGSAVE` writes a valid RDB. Freeze BIO (resume after CreateProcess). |
...
| # | PR title | Files / components | Depends on | Description |
|---|---------|--------------------|------------|-------------|
| 4.1 | `port: do_aofRewrite + MP-AOF` | `Win32_QFork_impl.c` `do_aofRewrite`; `aof.c` thin `#ifdef`; **no** 5.0 `aof_pipe_*` | 3.3 | Child builds `temp-rewriteaof-bg-<pid>.aof`; only `child_info_write` + server snapshot. |
